...
function renderStats() {
  ...
  let html = '<div class="space-y-4">' +
    ...
    '<button onclick="statsTab = \'item\'; renderStats();" class="flex-1 py-2 rounded-lg font-bold text-xs sm:text-sm transition ' +
    (statsTab === 'item' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600') + '">📦 항목별</button>' +
  ...
  } else if (statsTab === 'item') {
    html += renderStatsByItem(baseHistory);
  }
...
function renderStatsItemList() {
  ...
  baseHistory.forEach(h => {
    ...
    s.count++;
    const t = h.team || '(미지정)';
    if (!s.teams[t]) s.teams[t] = { qty: 0, cost: 0, count: 0 };
    s.teams[t].qty += h.qty || 0;
  ...
  if (searchTerm) {
    ...
    itemList = itemList.filter(it =>
      (it.name || '').toLowerCase().includes(lo) ||
...
function renderStatsByAnomaly() {
  ...
  // ─── 분류별 비교 섹션 — 사용자 요청으로 제거됨 ───
  // 코드 보존 (catOfHistory_ 함수는 다른 데서 안 씀, 미래 복원 시 참고용)
  // 분류별 변동 섹션 — 사용자 요청으로 UI 제거됨 (코드는 위에서 catThis/catPast 집계 자체가 빠짐)
