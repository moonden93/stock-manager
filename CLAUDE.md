...
3. **Phase 3: inventory/history 컬렉션화** — 1MB 한도 영구 해소 (2~3일)
4. **덴트웹 통합** — 환자 데이터 받으면 진행
5. **Audit log 조회 UI** — 화면에서 변경 이력 보기 (반나절)

> ❌ **Google 로그인(Phase 4)은 영구 보류** — 사용자 결정(2026-06-11): 평생 안 함. 인증은 device 단위 추적만 유지.

### 알려진 제약 / 미해결
- **Phase 2 검증 중** — 1주 정도 모니터링 후 단일문서 requests 필드 deprecated 가능
- **인증 없음** — device 단위만 추적 (Google 로그인은 영구 보류 결정)
...
2. **Phase 3.1 atomic stock wiring** (반나절) — 입고/반출 stock 변경을 increment로 (현 코드는 `adjustInventoryStock` 있으면 사용)
3. **덴트웹 통합** — 환자 데이터 받으면
...
2. **Phase 3.1 atomic stock wiring** — 같은 품목 동시변경 race 해소
3. **부분 렌더링 refactor** (D) — 현재 A+B로 충분한지 검증 후
...
## 22. 2026-06-11 작업: 주문 필요 강화 + 요청 수정 직접요청 풀 편집 + 전역 render 지연

### 22.1 주문 필요 섹션 풍부화
- **요청 일자 표시 + 오래된 순 정렬**: `📅 오늘 / 어제 / N일 전 요청 (날짜)` — 회색/앰버/오렌지/빨강(8일+)
- **장바구니 자동 제외**: orderCart에 담긴 itemId는 주문 필요에서 사라짐 ("정리" UX)
- **요청 메모/상세설명/사진 표시**: 직접요청은 customDescription/customImages, 일반요청은 r.memo
  - 같은 항목 여러 요청 → 중복 제거 후 모두 나열 (Set 사용)
- **요청자 표시**: `👤 팀 · 담당자` 형식, 여러 명이면 콤마 구분

### 22.2 요청 수정 모달 — 직접요청 모든 필드 편집 가능
- 이전: 수량만 + 메모 (헷갈렸음 — 직접요청 상세설명과 별개)
- 변경: 직접요청은 품목명/업체/단위/수량/상세설명/사진 **전부 수정 가능** (요청시 폼과 동일)
- 일반 inventory 요청은 기존대로 수량만
- **메모 (선택) 필드 제거** — 직접요청 상세설명과 통일
- 모달 부분 재렌더: `_renderEditMyRequestModal` (이미지 추가/제거 시)
- 이미지 편집 상태: `window._editReqImages[reqId] = []` (modal 동안만)

### 22.3 ⭐ 전역 render 지연 (큰 성능 개선)
**증상**: "한 번 클릭해도 안 되고 한 번 더 클릭해야" 입력/버튼 모두 영향
**원인**: 액션 핸들러가 renderRelease/Inbound 등 무거운 render를 즉시 호출 → 568 inventory + 1500+ history 전체 재구성 → 클릭 처리 도중 DOM 파괴 → 클릭 누락

**해결** (99-main.js, window load 후 적용):
```js
['Release', 'Manage', 'Inbound', 'Inventory', 'Stats', 'Settings'].forEach(name => {
  const fnName = 'render' + name;
  const original = window[fnName];
  const flag = '_' + fnName + 'Q';
  window[fnName] = function() {
    if (window[flag]) return;  // 같은 프레임 내 중복 무시
    window[flag] = true;
...
효과:
- 클릭/입력 핸들러 즉시 종료 (render 안 기다림)
- 같은 프레임 내 여러 render 호출 → 한 번만 실행 (coalesce)
- render는 다음 프레임에 → 클릭 이벤트 완전히 처리된 후 DOM 갱신
- **부분 갱신 (renderXxxItems) 은 wrapping 안 함** — IME 안전성 위해 즉시 실행

### 22.4 기타 작은 변경
- 입고 내역 수정 모달 ghost tap 가드 (`closeModalFromBackdrop` + `markModalOpened`)
- 요청 수정 모달도 같은 가드 추가
- B (버튼 disabled) 500ms → 250ms
- addCustomItemToCart: `renderCartBar` 즉시 + `renderRelease` setTimeout(0) 으로 미룸
- 요청 탭에 숨김 항목 시각 표시 (`🙈 숨김` 배지 + 노란 경고 + 회색 배경)
- 비밀번호 모달 라벨: `입고` → `주문/입고`

### 22.5 직접요청 picker 모달 link 버그 해소
13-settings.js와 8-manage.js 양쪽에 `linkCustomToInventory` 동명함수 존재해서 충돌
→ 8-manage.js 함수 `linkCustomReqToInventoryItem` 으로 rename
→ data-attr + addEventListener 패턴 (inline onclick 회피)

### 22.6 다음 세션 진행 사항
**진행 중 — 마무리 필요**:
- 6/11 session에서 작업 중단됨
- 새 세션에서 다음 프롬프트로 이어가기 권장 (별도 파일: `_NEXT_SESSION_PROMPT.md`)

**우선순위 점검**:
1. 운영 1주 후 성능 (22.3 효과) 검증
2. 장바구니 동기화 race condition (한쪽 삭제 + 한쪽 추가) 실사용 시 발생하는지 모니터링
3. implant-manager: 단가/부족기준 입력 시작

> ❌ **Google 로그인(Phase 4)은 영구 보류** — 사용자 결정(2026-06-11): 평생 안 함.

### 22.7 의도된 한계 (현재 디자인)
- **장바구니 race**: 한쪽 기기 삭제 + 다른 쪽 추가 동시면 추가가 이김 (드물어 OK)
- **render 지연 1프레임 (~16ms)**: 거의 인지 불가
- **임플란트 매니저 백업 없음**: 별도 Firebase라 stock-manager 시스템 미적용

---

## 23. 2026-06-11 후속: 사진 압축 + 주문 참고정보 + 다중기기 race 3종

### 23.1 직접요청 사진 첨부 안 되던 문제
- 원인 1: 5MB 하드컷 — 폰 사진(5~12MB)이 그냥 건너뛰어짐 (미리보기도 안 뜸)
- 원인 2: base64 원본이 Firestore 문서 1MB 한도 초과 → 컬렉션 저장 실패
- 해결: `compressImageFile()` (7-release.js) — canvas 1024px / JPEG 0.55 (~40~120KB)
  - 요청 생성(handleCustomImages) + 요청 수정(handleEditReqImages) 양쪽 적용
  - 원본 허용 25MB, 이미지 외/압축실패 폴백
- 요청 수정에서 **사진만** 바꿔도 저장되게: anyChange에 이미지 변경 감지 추가
  (이전엔 사진만 바꾸면 즉시 upsert 안 돼 listener echo가 덮어씀)

### 23.2 주문 시 요청 참고정보(상세설명/사진/요청자) 전 과정 표시
- `getOrderRefForItem(itemId)` — 대기 요청에서 수집 (9-inbound.js)
- `orderRefBlockHtml_` / `orderRefHasContent_` 공통 렌더
- 표시 4곳: 장바구니 담기 다이얼로그 / 장바구니 / 주문 카드(단일·묶음)
- 주문 등록 시 `order.items[].ref`로 **스냅샷** — 요청 처리돼도 주문 카드에 유지

### 23.3 ⭐ 다중기기 동기화 race 3종 (중요)
**증상**: 요청 취소해도 새로고침하면 부활 / 다른 PC에서 담은 카트 안 사라짐 / 화면이 옛것

**1) 요청 취소 부활 (가장 심각)**
- 원인: saveAll의 Phase 2 디바운스 배치가 매번 `requests` **전체를 재기록**.
  다른 기기가 stale 메모리(pending)로 방금 한 취소를 덮어씀.
- 해결: hash diff 캐시 (`window._reqHashCache`) — 이 기기가 **실제 바꾼 항목만** upsert.
  - listener 메모리 교체 시 캐시를 클라우드 기준선으로 갱신 (`_reqHashCacheReady`)
  - 캐시 준비 전(초기 로드)엔 배치 skip (즉시 upsert가 커버)
- ⚠️ **교훈**: 컬렉션 동기화에서 "전체 배열 재기록"은 절대 금지. 항상 변경분만.
  (inventory/history는 Phase 3에서 이미 hash diff. requests만 누락돼 있었음)

**2) 장바구니 cross-device wipe**
- 원인: `orderCart`를 매 저장마다 통째로 덮어써 빈/stale 카트가 다른 기기 카트 wipe.
- 해결: `window._orderCartDirty` — 이 기기가 카트 변경 시에만 단일문서에 씀.
  (saveOrderCartItem/removeOrderCartItem(ById)/clearOrderCart/submitOrder에 플래그)

**3) 속도 최적화가 동기화 화면 반영을 막음**
- 원인: `debouncedReRenderCurrentTab`이 클릭 800ms 내면 **무한 재연기** →
  활발히 쓰는 동안 다른 기기 변경/삭제가 화면에 안 뜸.
- 해결: 1.5초 넘게 밀리면 강제 갱신 (`_reRenderFirstRequestedAt`).

### 23.4 다음 세션이 알아야 할 것
- requests 배치는 이제 hash diff. 새 쓰기 경로는 **즉시 upsertRequestDoc** 유지 필수.
- orderCart 변경 시 반드시 `window._orderCartDirty = true` 설정 후 saveAll.
- 사진은 항상 compressImageFile 통과 (직접 readFileAsBase64로 큰 이미지 저장 금지).

---

_마지막 갱신: 2026-06-11 (사진 압축 + 주문 참고정보 + 다중기기 race 3종 수정)_
